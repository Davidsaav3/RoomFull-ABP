...
@Component({
  selector: 'app-admin-suscripciones-editar',
  templateUrl: './admin-suscripciones-editar.component.html',
  styleUrls: ['./admin-suscripciones-editar.component.css']
})
export class AdminSuscripcionesEditarComponent implements OnInit {
  nombre = '';
  descripcion = '';
  precio = 0;
  modelos = 0;
  ...
  ngOnInit(): void {
    ...
    this.tipoSuscripcionService.cargarTipoSusId(this.router.routerState.snapshot.url.split('/')[4]).subscribe(
      {
        next: (res:any) => {
          console.log(res)
          this.suscripcion = res.tipoSus;

          this.uid = res.tipoSus.uid;
          this.actualizarForm();
        },
        error: (err:any)=> {
          console.log(err);
  ...
  actualizarForm(){
    ...
    suscribir() {
      ...
      this.tipoSuscripcionService.actualizarTipoSus(this.uid, obj).subscribe(
        {
          next: () => {
            Swal.fire({
