...
export class TModelo extends TEntidad{
  ...
  fondos : [] = []; // fondos skybox

  colores : glMatrix.vec3[] = [];
  ...
  public constructor(){
    ...
    // Ambiente
    this.colores.push(glMatrix.vec3.fromValues(0.949,0.925,0.898)); // Amanecer
    this.colores.push(glMatrix.vec3.fromValues(1.0,1.0,1.0)); // Medio dia
    this.colores.push(glMatrix.vec3.fromValues(0.98,0.839,0.647)); // Atardecer
    this.colores.push(glMatrix.vec3.fromValues(0.384,0.616,0.761)); // Noche
  }
  ...
  public draw(gl, buffers, programInfo, programShadow, projectionMatrix, modelMatrix, normalMatrix, nombre, posicionesLucesEscena, cameraPos, ambienteColor, fondo): void {
    ...
    gl.uniform3fv(
      programInfo.uniformLocations.lightsPositions,
      posicionesLucesEscena[0]
    );

    gl.uniform3fv(
      programInfo.uniformLocations.lightColor,
      this.colores[ambienteColor]
    );
    ...
    if(!this.skybox){
      ...
      gl.bindTexture(gl.TEXTURE_2D, this.texturas[fondo]);
    }
...
setColorAttribute(gl, buffers, programInfo) {
  ...
  getBuffers(gl) {
    ...
    if(!this.skybox){
        ...
        this.fondos.forEach((img : any, index: number) =>{
          console.log(index)
          this.texturas.push(this.loadTextureReturn(gl,URL.createObjectURL(base64StringToBlob(img, "image/jpeg"))));
        })
    }
