...
@Component({
  selector: 'app-user-profile-guard',
  templateUrl: './user-profile-guard.component.html',
  styleUrls: ['./user-profile-guard.component.css']
})
export class UserProfileGuardComponent implements OnInit {
  ...
  private maximoModelos: any = 8;
  valor:any = 1;
  public numGuardados:any;
  ...
  ngOnInit(): void {
    ...
    //this.uidusuario = this.activatedRoute.snapshot.params.uid;
        ...
        this.usuarioService.cargarUsuario(this.uid).subscribe(
          {
            next: (res:any) => {
              ...
              if(this.nameuser==this.tokenUserName){
                ...
                this.escenaService.obtenerEscenasFiltrosPerfil(this.valor, this.nombre, this.desde, "guardados").subscribe({
                  next:(res:any)=>{

                    this.ultimaPage = Math.ceil(res.page.maximo/this.maximoModelos);
